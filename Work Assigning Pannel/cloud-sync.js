    ...
    init() {
      ...
      if (config && config.apiKey && config.projectId && window.firebase) {
        try {
          ...
          const statusEl = document.getElementById('cloudStatusBadge');
          if (statusEl) {
            statusEl.className = 'badge badge-approved';
            statusEl.innerHTML = '🟢 Connected to Firebase Cloud (' + config.projectId + ')';
          }
        } catch (err) {
          ...
          const statusEl = document.getElementById('cloudStatusBadge');
          if (statusEl) {
            statusEl.className = 'badge badge-rejected';
            statusEl.innerHTML = '🔴 Cloud Connection Error';
          }
        }
      } else {
        const statusEl = document.getElementById('cloudStatusBadge');
        if (statusEl) {
          statusEl.className = 'badge badge-underreview';
          statusEl.innerHTML = '🟡 Local Storage Mode (Cloud Disconnected)';
        }
      }
    },
        ...
        if (doc.exists) {
          ...
          if (cloudData && cloudData.payload) {
            ...
            if (localDataStr !== cloudDataStr) {
              ...
              if (window.renderTaskTable) window.renderTaskTable();
